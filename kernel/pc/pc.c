...
// 根据进程号shift进程，将要提升优先级的进程从原来的优先级调度列表中移除，放入新的调度队列中
//返回0表示执行成功，否则表示执行失败
int pc_shift(pid_t pid) {
	int res;
	struct task_struct* task, *sched_node;

	//idle进程不能被shift
	if (pid == IDLE_PID) {
		kernel_printf("You are shifting idle process!!!\n");
		return 1;
	}

	//init进程不能被shift
	if (pid == INIT_PID) {
		kernel_printf("You are shifting init process:kernel shell!!!\n");
		return 1;
	}

	//进程不能shift自身
	if (pid == current_task->pid) {
		kernel_printf("You can't shift current task!\n");
		return 1;
	}

	disable_interrupts();

	//通过进程id号检查要shift的进程是否存在
	res = pid_check(pid);
	if (res == 0) {
		kernel_printf("pid not found!\n");
		goto error_0;
	...
	// 通过pid获取要被shift的的进程的task_struct结构
	task = find_in_tasks(pid);
	...
	//已经达到最高优先级的进程无法被shift
	if (task->sched_lv <= SCHED_LV1)
	{
		kernel_printf("this task has enough priority!\n");
		return 1;
	}
	...
	if (task != sched_node) {
		kernel_printf("pc_shift: task != sched!!!!!!!BUG EXIST!!!!\n");
		goto error_2;
	}
#endif
	remove_sched(task);
	task->sched_lv--;
	task->time_cnt = sched_time_cnt[current_task->sched_lv];
	add_sched_back(task, task->sched_lv);
	enable_interrupts();
	return 0;
	...
	error_2 :
			while (1)
				;
		error_1:
			kernel_printf("pc_shift: task struct not found, There must be bug!!!\n");
#endif
...
int pc_send(char* args) {
	...
	for (i = 0; i < strlen(args); i++)
		if (args[i] == ' ')
			break;
	for (j = 0; j < i; j++)
		pid = pid * 10 + args[j] - 48;
	char* mes = (char*)kmalloc(sizeof(char) * ((strlen(args) - i)));
	for (j = i + 1; j < strlen(args); j++)
		mes[j - i - 1] = args[j];
	mes[strlen(args) - i - 1] = '\0';


	disable_interrupts();

	//通过进程id号检查要发送信息的进程是否存在
	res = pid_check(pid);
	if (res == 0) {
		kernel_printf("pid not found!\n");
		goto error_0;
	...
	messagetail++;
	kernel_strcpy(message[messagetail], mes);
	sender[messagetail] = pid;
	...
	if (task != sched_node) {
		kernel_printf("pc_send: task != sched!!!!!!!BUG EXIST!!!!\n");
		goto error_2;
	}
#endif
	...
	error_2 :
			while (1)
				;
		error_1:
			kernel_printf("pc_send: task struct not found, There must be bug!!!\n");
#endif
...
//receive信息
int pc_receive(pid_t pid) {
	int res;
	struct task_struct* task, *sched_node;
	disable_interrupts();

	//通过进程id号检查要接受信息的进程是否存在
	res = pid_check(pid);
	if (res == 0) {
		kernel_printf("pid not found!\n");
		goto error_0;
	...
	{
		kernel_printf("there is no message that can be used!\n");
		return 1;
	}

	task->messageid = messagehead; 
